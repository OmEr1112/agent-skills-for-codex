...
- **If you find yourself applying this skill from inside a Codex subagent context:** the preferred path is to surface to the user that doubt-driven review belongs in the main session and let the main session handle it. As a last resort only, a degraded self-questioning fallback exists — rewrite ARTIFACT + CONTRACT as a fresh self-prompt with a hard mental separator from your prior reasoning, and walk Steps 1–5. This is **not fresh-context review** (you carry your own context with you), so flag the result as degraded and prefer escalation whenever the user is reachable.
...
In Codex, use `spawn_agent` with a role such as `code-reviewer`, `security-auditor`, or `test-engineer` when the user has authorized subagent work. These roles are registered through `agents.<name>` entries in `~/.codex/config.toml` or trusted project `.codex/config.toml`.
...
> *"Single-model review complete. Want a second opinion? Options: another Codex model/profile, manual external review (you paste it elsewhere), or skip."*
...
5. Mind shell escaping. If the artifact contains quotes, `$(...)`, or backticks, prefer stdin or a heredoc over inline prompt arguments. When in doubt, ask the user to confirm the invocation before running it.
